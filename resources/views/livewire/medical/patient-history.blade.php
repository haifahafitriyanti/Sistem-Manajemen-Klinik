        ...
        <div class="space-y-3">
            @foreach($appointments as $apt)
                <div wire:key="history-{{ $apt->id }}"
                    ...
                    <button wire:click="toggle({{ $apt->id }})"
                        ...
                        {{-- Chevron --}}
                        <svg class="w-4 h-4 text-slate-400 shrink-0 transition-transform {{ ($expanded[$apt->id] ?? false) ? 'rotate-180' : '' }}"
                    ...
                    {{-- Expanded details --}}
                    @if($expanded[$apt->id] ?? false)
                        <div class="border-t border-slate-100 px-6 py-5 bg-slate-50 space-y-4">
                            @if($apt->complaint)
                                <div>
